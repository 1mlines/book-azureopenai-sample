#!/bin/bash
# Azure에서 생성한 서비스 주체(service principal)의 <object_id>로 역할 할당하기
#
# Prerequirement: 서비스 주체를 생성하고 개체 ID(object id)를 취득해 놓아야 한다. 
...
# memo: https://docs.microsoft.com/ko-kr/azure/role-based-access-control/built-in-roles
