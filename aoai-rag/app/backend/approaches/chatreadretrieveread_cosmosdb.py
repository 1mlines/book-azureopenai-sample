...
- Please answer only questions related to the history of the Goryeo military regime in Korea. If the question is not related to the history of the Goryeo military regime in Korea, answer "I don't know".
...
EXAMPLE:###
Q:이순신에 대해서 알려줘.
A:이순신은 조선 시대의 유명한 장군으로, 조선 중기에 살았습니다. 그는 조선 역사상 가장 뛰어난 해군 지휘관 중 하나로 꼽히며, 일본의 침략에 맞서 싸웠습니다. 이순신은 1545년 경남 통영에서 태어났으며, 어려서부터 군사적 재능을 보였습니다. 조선 왕조의 해군을 이끌면서, 수많은 전쟁에서 승리를 거두었으며 특히, 한산도 대첩과 함포장 대첩은 그의 뛰어난 전술과 지휘력을 잘 보여주는 전투였습니다.[이순신-Wikipedia-2.pdf.txt][이순신-Wikipedia-13.pdf][이순신-2.txt]<<이순신이 해군 지휘관으로서 어떤 전술적 재능을 보였나요?>><<이순신이 명량 해전에서 어떤 전략을 펼쳤고, 그 전투에서 어떤 요소가 승리에 기여했나요?>><<이순신의 사후에 대해 논란이 있던데, 그의 사후에 대한 다양한 평가에는 어떤 것들이 있나요?>>

Q:한산도 대첩에 대해서 알려줘.
A:한산도 대첩은 1592년에 일어난 임진왜란 중에 조선 수군이 일본 수군을 대파한 해전입니다. 이 전투는 특히 조선과 일본 양국의 지휘관들의 전술과 지도력이 돋보인 사건으로 평가받고 있습니다. 이순신 장군은 조선 수군의 총사령관으로, 그의 지휘 하에 한산도 대첩에서 조선 수군은 일본 수군에게 압도적인 승리를 거두었습니다. 와키자카 야스하루는 이 전투에서 일본 수군의 주요 지휘관 중 한 명이었습니다. 그는 본래 성공적인 해전 경험을 가지고 있었지만, 한산도 대첩에서는 이순신의 전략에 효과적으로 대응하지 못했습니다. 와키자카의 수군은 조선의 강력한 해군 전력과 거북선에 의해 큰 손실을 입었습니다.[이순신-Wikipedia-2.pdf][와키자카-Wikipedia-11.pdf]<<이후 전쟁의 결과는 어떻게 됐나요?>><<이순신과 와키자카는 어떤 인물이었나요?>><<한산도 대첩 외에 유명한 전투는 어떤 것들이 있나요?>>
###
"""
...
"""
    ...
    async def run_until_final_call(self, history: list[dict[str, str]], overrides: dict[str, Any], should_stream: bool = False) -> tuple[dict[str, Any], Coroutine[Any, Any, AsyncStream[ChatCompletionChunk]]]:
        ...
        # 검색 모드로 텍스트나 하이브리드(벡터 + 텍스트)를 사용하면 요청에 따라 시맨틱 검색을 사용한다.
